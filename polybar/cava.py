...
bytetype, bytesize, bytenorm = (
    ("H", 2, 65535) if (CAVA_BIT_FORMAT == "16bit") else ("B", 1, 255)
)
...
def run():
    ...
    cavaProcess = subprocess.Popen(
        ["cava", "-p", CAVA_CONFIG_PATH], stdout=FNULL, stderr=subprocess.STDOUT
    )
...
def printTestData():
    ...
    for bar_threshold in BAR_CHARACTERS:
        print("{:06.2f}: {}".format(bar_threshold, BAR_CHARACTERS[bar_threshold]))
